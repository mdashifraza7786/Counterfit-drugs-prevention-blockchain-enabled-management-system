...
import { InboxIcon, CubeIcon, ClipboardDocumentCheckIcon } from '@heroicons/react/24/outline';
...
const PharmacyDashboard = () => {
  ...
  const fetchBoxes = async () => {
    try {
      const response = await api.get('/pharmacy/boxes');
  ...
  const getStatusColor = (status) => {
    const colors = {
      'AT_PHARMACY': 'bg-blue-100 text-blue-800',
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-6 lg:p-8">
          <div className="mb-8">
            ...
            <p className="text-gray-600">Manage inventory and activate units for sale</p>
          </div>
          ...
          <div className="mb-6 border-b border-gray-200">
            <nav className="flex space-x-8">
              {[
                { id: 'boxes', label: 'Boxes', icon: InboxIcon },
                { id: 'units', label: 'Inventory', icon: CubeIcon },
                { id: 'receive', label: 'Receive Box', icon: ClipboardDocumentCheckIcon },
              ].map(tab => {
                const Icon = tab.icon;
                return (
                  <button
          ...
          {activeTab === 'units' && (
            <div className="card">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-gray-900">Inventory ({units.length} units)</h2>
                <button onClick={() => navigate('/pharmacy/activate')} className="btn-primary">
                  Activate Unit
                </button>
              </div>
              {units.length === 0 ? (
